...
(:action LOAD
      :parameters (?p - package ?t - truck ?l - location ?l2 - location ?c1 ?c2 ?c3 ?c4 - count )
...
(:action UNLOAD
      :parameters (?p - package ?t - truck ?l - location ?c1 ?c2 - count)
      :precondition (and (at ?t ?l) 
                                    (in ?p ?t)
                                    (g_at ?p ?l)
                                    (next_cnt ?c1 ?c2)
...
(:action DRIVE
      :parameters (?t - truck ?l1 - location ?l2 - location ?fuelpost - fuellevel
                            ?fueldelta - fuellevel ?fuelpre - fuellevel ?c - count
                          )
